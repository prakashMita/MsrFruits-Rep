...
const [orderTypeFilter, setOrderTypeFilter] = useState(''); // Order type filter
  ...
  // // Filter logic for both Mobile and UserName
  // const filteredCustomers = uniqueCustomers.filter((customer) => {

  //   const matchesUserName = customer.UserName.toLowerCase().includes(userNameFilter.toLowerCase());
    
  //   return (
  //     (!mobileFilter || matchesMobile) && 
  //     (!userNameFilter || matchesUserName)
  //   );
  // });

  const filteredCustomers = customers.filter(customer => {
    const matchesOrderId = orderIdFilter ? customer.OrderId.toString().includes(orderIdFilter) : true;
    const matchesMobile = mobileFilter ? customer.UserMobile.toLowerCase().includes(mobileFilter.toLowerCase()) : true;
    ...
    const matchesDateRange = fromDate && toDate ? customerDate >= fromDate && customerDate <= toDate : true;
  
    // Check if we are filtering by today's date
    const matchesTodayDate = isFilteringByToday ? customerDate === new Date().toISOString().split('T')[0] : true;
  
    // Apply all filters
    return matchesOrderId &&
           matchesMobile &&  // Added mobile filter check
           matchesUserName &&
           matchesStatus &&
           matchesOrderType &&
           matchesDateRange && 
...
//   setTotalPrice(total);
// }, [filteredCustomers]);
// Step to filter unique customers based on UserMobile
// const uniqueCustomers = filteredCustomers.filter(
//   (customer, index, self) => index === self.findIndex((c) => c.UserMobile === customer.UserMobile)
// );

// const uniqueMobileCount = uniqueCustomers.length; // Count of unique UserMobile entries

// Step 1: Create a frequency count of UserMobile in filteredCustomers
const mobileCountMap = filteredCustomers.reduce((acc, customer) => {
  acc[customer.UserMobile] = (acc[customer.UserMobile] || 0) + 1;
  return acc;
}, {});

// Step 2: Filter unique customers based on UserMobile
// const uniqueCustomers = filteredCustomers.filter(
//   (customer, index, self) => index === self.findIndex((c) => c.UserMobile === customer.UserMobile)
// );
...
// Step 1: Create a map to sum OverAllAmount and count orders for each UserMobile
const mobileDataMap = filteredCustomers.reduce((acc, customer) => {
  const { UserMobile, OverAllAmount } = customer;
  ...
  acc[UserMobile].orderCount += 1;
  acc[UserMobile].totalAmount += parseFloat(OverAllAmount) || 0; // Ensure OverAllAmount is a number
...
// State to track the mobile number filter


// Step 1: Filter unique customers based on UserMobile
const filteredUniqueCustomers = uniqueCustomers.filter((customer) => 
  customer.UserMobile.toLowerCase().includes(mobileFilter.toLowerCase()) // Apply case-insensitive filter
);
  return (
    <div>
      <div className='row'>
        ...
        <div className='col-6 my-auto text-end'>
          <div className="text-right my-auto pe-4">
            <h3><strong className='my-auto'>Total Orders:<span className='text-warning'> {uniqueMobileCount}</span></strong></h3>
          </div>
        </div>
      </div>
      {/* price report */}
...
<div className='pt-2'></div>
  ...
  <div className="row">
    <div className="form-group col-md-3 col-3">
      <label htmlFor="orderIdFilter">Mobile</label>
      <input
      id="mobileFilter"
      type="text"
...
{/* <p>Total Unique Customers by Mobile: {uniqueMobileCount}</p> */}
        ...
        <table className="table">
          <thead className="table-dark">
            <tr>
              <th>Name</th>
              <th>Mobile</th>
              <th className="text-center">OrderCount</th>
          ...
          <tbody className="postablebody">
            {uniqueCustomers.map((customer, index) => (
              <tr key={index}>
                ...
                <td className="text-center">{mobileDataMap[customer.UserMobile].orderCount || 'N/A'}</td>
                <td className="text-center">{mobileDataMap[customer.UserMobile].totalAmount.toFixed(2) || 'N/A'}</td>
              </tr>
            ))}
          </tbody>
        </table>
